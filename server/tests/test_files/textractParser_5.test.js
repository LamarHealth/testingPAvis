...
const data = parseTextract(masters5TextractJSON);

console.log(data);

const testForKeys = Object.keys(masters5Fields).map((key, i) => {
  return test(`Mast5 key TEST ${i + 1}: ${key}`, () => {
    expect(data[key]).toBeDefined();
  });
});

const testForValue = Object.keys(masters5Fields).map((key, i) => {
  ...
  return test(`Mast5 value TEST ${i + 1}: ${key}: ${
    masters5Fields[key]
  } `, () => {
    expect(masters5Fields[key]).toMatch(valueToMatch);
  });
});
